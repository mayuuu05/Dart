import 'dart:io';
import 'bankFctoryConstructor.dart';
...
void main() {
  for (int i = 0; i < bankData.length; i++) {
    BankMOdel Bank = BankMOdel.fromDetailsOfBank(DetailsOfBank: bankData[i]);
    BankList.add(Bank);
  }
  for (int i = 0; i < BankList.length; i++) {
    print('isActive : ${BankList[i].isActive} ' +
        ' balance : ${BankList[i].balance} ' +
        ' age : ${BankList[i].age}' +
        'eyeColor : ${BankList[i].eyeColor} ' +
        'name : ${BankList[i].name}' +
        'gender :${BankList[i].gender}' +
        'company : ${BankList[i].company}' +
        'email :${BankList[i].email}' +
        'phone : ${BankList[i].phone}' +
        'address : ${BankList[i].address}');
        print('\n');
  }
}
